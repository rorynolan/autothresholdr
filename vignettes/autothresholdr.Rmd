...
```{r guess twenty}
suppressWarnings(image(img > 20, method = "r"))
```
...
auto_thresh_mask(img, "huang") %>% {suppressWarnings(image(.))}
auto_thresh_apply_mask(img, "huang") %>% image()
```
...
image(img[, , 1])
mean_stack_thresh(img, "tri")[, , 1] %>% image()
med_stack_thresh(img, "tri")[, , 1] %>% image()
```
