...
```{r thresh mask apply}
auto_thresh(img_matrix, "huang")
auto_thresh_mask(img_matrix, "huang") %>% display()
auto_thresh_apply_mask(img_matrix, "huang") %>% display()
```
...
dim(img_3d)
display(img_3d[, , 1])
mean_stack_thresh(img_3d, "tri")[, , 1] %>% display()
med_stack_thresh(img_3d, "tri")[, , 1] %>% display()
```
